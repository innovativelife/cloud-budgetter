...
type SummaryTab = 'overview' | 'byMonth';

export function SummaryPage() {
  const { state } = useAppState();
  const { services, budgetConfig, budgetData } = state;
    ...
    return (
      <div>
        ...
        <div className="text-center py-12 bg-gray-50 rounded-lg border border-dashed border-gray-300">
          <p className="text-gray-500">No services configured. Add services and enter budget data first.</p>
        </div>
      </div>
    );
  }
    ...
    <div>
      ...
      {activeTab === 'byMonth' && (
        <div>
          {/* Stacked Bar Chart */}
          <div className="mb-8">
            <h3 className="text-lg font-semibold text-gray-900 mb-3">Monthly Total</h3>

            {/* Legend */}
            <div className="flex flex-wrap gap-3 mb-4">
              {services.map((service, idx) => (
                <div key={service.id} className="flex items-center gap-1.5 text-xs text-gray-700">
                  <span className={`w-3 h-3 rounded-sm inline-block ${SERVICE_COLORS[idx % SERVICE_COLORS.length]}`} />
                  {service.name}
                </div>
              ))}
              ...
              <table className="text-xs w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    ...
                    <th className="text-right py-2 px-2 font-medium text-gray-600 min-w-[100px]">Total</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {services.map((service, idx) => (
                ...
                <tfoot>
                  <tr className="border-t-2 border-gray-300 font-semibold">
                    ...
                    <td className="py-2 px-2 text-right">{formatCurrency(grandTotal)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
